...
export function ReadView({
  ...
  const filteredPapers = readPapers.filter(
    (paper) =>
      paper.title.toLowerCase().includes(searchQuery.toLowerCase()) ||
      paper.authors.toLowerCase().includes(searchQuery.toLowerCase())
  );

  const displayPapers = searchQuery ? filteredPapers : readPapers;

  return (
    <div className="p-8 space-y-8">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-bold text-gray-900 mb-2">Read</h1>
          <p className="text-gray-600">
            Papers you have finished {displayPapers.length > 0 && `(${displayPapers.length})`}
          </p>
        </div>
        <button
          onClick={onAddPaper}
      ...
      {displayPapers.length === 0 ? (
        <div className="text-center py-12 bg-gray-50 rounded-lg">
          ...
          <h3 className="text-xl font-semibold text-gray-900 mb-2">
            {searchQuery ? 'No papers found' : 'No Papers Read Yet'}
          </h3>
          <p className="text-gray-600 mb-6">
            {searchQuery
              ? `No papers match "${searchQuery}"`
              : 'Mark papers as read to see them here'}
          </p>
        </div>
      ) : (
